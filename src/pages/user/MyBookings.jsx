...
export default function MyBookings({
  ...
  if (role === 'guest') return <div className="page scroll-animate" style={{
    textAlign: 'center'
  }}>
  ...
  const currentBookings = tab === 'rentals' ? filteredRentals : tab === 'trips' ? tripBookings : [];
    ...
    maxWidth: 640
      ...
      alignItems: 'center'
        ...
        gap: 8
          ...
          id: 'rentals',
          label: 'Rentals'
        }].map(t => <div key={t.id} className={`pill-tab ${tab === t.id ? 'active' : ''}`} onClick={() => {
          setTab(t.id);
      ...
      {tab === 'rentals' && <div className="pill-nav" style={{
      marginTop: -6,
      marginBottom: 20
    }}>
          {[{
        ...
        id: 'returned',
        label: 'Returned'
      }].map(f => <div key={f.id} className={`pill-tab ${rentalFilter === f.id ? 'active' : ''}`} onClick={() => setRentalFilter(f.id)}>
              {f.label}
            </div>)}
        </div>}
      {tab === 'trips' && <div className="pill-nav" style={{
      marginTop: -6,
      marginBottom: 20
    }}>
          {[{
        ...
        id: 'past',
        label: 'Past'
      }].map(f => <div key={f.id} className={`pill-tab ${tripFilter === f.id ? 'active' : ''}`} onClick={() => setTripFilter(f.id)}>
              {f.label}
            </div>)}
        </div>}

      {currentBookings.map((b, i) => <div key={b.id} className="booking-item ticket-card scroll-animate" style={{
      '--delay': `${i * 40}ms`
    }}>
          <div className="booking-header">
            ...
            <span className={`badge ${b.status === 'Confirmed' ? 'badge-green' : b.status === 'Completed' || b.status === 'Returned' ? 'badge-purple' : 'badge-amber'}`}>
              {b.status}
            </span>
          </div>
          ...
          {(b.status === 'Confirmed' || b.status === 'Completed') && b.type === 'ticket' && <div style={{
        marginTop: 12,
        ...
        paddingTop: 12
                ...
                {qrOpen === b.id && <div className="qr-reveal">
                    <div style={{
            fontSize: 12,
            color: 'var(--text-2)'
          }}>
